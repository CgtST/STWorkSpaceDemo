...
@property(nonatomic,readwrite,retain,nonnull) UIButton *cancleButton;  //取消按钮

@end

@implementation CM2ButtonAlertView
...
-(nonnull instancetype)initWithTitle:(nullable NSString*)title Content:(nonnull NSString*)contentStr
{
    ...
    if(nil != self)
    {
        ...
        //内容
        {
            CGFloat contentHeight = self.bounds.size.height - ypos - 1 - K_ButtonHeight;
        ...
        //按钮
        {
            ...
            sepView = [[UIView alloc] initWithFrame:CGRectMake(K_Space + contentWidth/2, ypos, 1, K_ButtonHeight)];
            [self addSubview:sepView];
            [mutableArr addObject:sepView];
            
            __weak CM2ButtonAlertView *alert = self;
            self.cancleButton = [CMAlertButton buttonWithType:UIButtonTypeCustom];
            [((CMAlertButton*) self.cancleButton) setDismissBlock:^
            {
                [alert dismiss];
            }];
            self.cancleButton.frame = CGRectMake(K_Space, ypos, contentWidth/2, K_ButtonHeight);
            [self.cancleButton setTitle:@"取消" forState:UIControlStateNormal];
            [self.cancleButton addTarget:self action:@selector(btnAction) forControlEvents:UIControlEventTouchUpInside];
            ...
            self.sureButton.frame = CGRectMake(K_Space + contentWidth/2, ypos, contentWidth/2, K_ButtonHeight);

...
- (UIColor *)getColorOfPoint:(CGPoint)point
{
    ...
    UInt8 pixelsPerPoint = 4;//png
    UInt32 pixelPosition = ((self.size.width * point.y) + point.x) * pixelsPerPoint;
    UInt8 red   = data[pixelPosition];
    UInt8 green = data[pixelPosition + 1];
    UInt8 blue  = data[pixelPosition + 2];
    UInt8 alpha = data[pixelPosition + 4];
    UIColor *color = [UIColor colorWithRed: red / 255 green: green / 255 blue: blue / 255 alpha: alpha / 255];
    return color;
}
